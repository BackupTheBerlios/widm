/*
 * exitorder - code from widm1.x -- I have no idea how it does what it does,
 *             but it seems to work allright
 */
...
#if 1
	if(!_finite(x)) {
	    if (_isnan(x)) return x;     /* erf(nan)=nan */
...
#if 1
	if(!_finite(x)) {
	    if (_isnan(x)) return x;     /* erfc(nan)=nan */
...
TStringList *exitorder(TStringList *namen, String uitvaller) {
     TStringList *ret = new TStringList(), *nonoutnames = new TStringList();
     int x, t = 1, num = namen->Count;
     double mean = (num * 0.75) - .5;
     double sd = (mean - 1) / 4;
     if (sd == 0) sd = 0.5;
     bool have_exit = true;
     double exit, total; int randn;

     randomize();
     for (x = 0; x < num; x++)
        if (namen->Strings[x] != uitvaller) nonoutnames->Add(namen->Strings[x]);
     while (t <= num) {
       /* black magic ... (with a doubtful scientific background though) */
       exit = normal_cdf(t, mean, sd) - normal_cdf(t-1, mean, sd);
       total = normal_cdf(num, mean, sd) - normal_cdf(t-1, mean, sd);
       randn = rand() % (int)(total * 10000);
       if (have_exit && randn <= (int)(exit * 10000)) {
         ret->Add(uitvaller);
         have_exit = false;
       } else {
         /* pick randomly */
